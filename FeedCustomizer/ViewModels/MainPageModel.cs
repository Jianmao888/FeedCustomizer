    ...
    public class MainPageModel
    {
        ...
        public async Task Init()
        {
            ...
            else
            {
                // 如果没有，则进行从磁盘初始化的逻辑
                // 更新或者复制源提供程序至用户数据目录
                bool isUpToDate = await Task.Run(
                    async () => await ResourcesCopier.IsResourceUpToDate());
                if (!isUpToDate)
                {
                    if (IsFeedProviderEnabled)
                    {
                        ...
                        await Task.Run(
                            async () => await ResourcesCopier.ResourcesCopyAsync());
            ...
            if (IsFeedProviderEnabled.Value &&
                !await Task.Run(ResourcesCopier.IsRegisteredProviderCurrent))
            {
                await PackageInstaller.UninstallFeedProvider();
