...
namespace FeedCustomizer.Core.Tools
{
    public class ManifestXmlService
    {
        ...
        private static List<Feed> ParseDefinitions(XDocument doc)
        {
            ...
            foreach (var element in elements)
            {
                var def = new Feed
                {
                    ...
                    ImagePath = FromPackageIconUri(GetAttr(element, "Icon"))
                };
                feedItems.Add(def);
            }
        ...
        private static XElement CreateProviderExtension(XElement template, Feed feed)
        {
            ...
            // The manifest schema lists these provider attributes as required;
            // retaining them also keeps compatibility with older Board builds.
            ...
            provider.SetAttributeValue("Icon", ToPackageIconUri(icon));
        ...
        private static XElement CreateEmptyProviderExtension(XElement template)
        {
            ...
            provider.SetAttributeValue("Icon", ToPackageIconUri("Assets\\StoreLogo.scale-200.png"));
            provider.Attribute("SettingsUri")?.Remove();
            definitions.RemoveNodes();
            return extension;
        }
        ...
        private static XElement CreateDefinitionElement(Feed def)
        {
            ...
            element.Add(
                ...
                new XAttribute("Icon", ToPackageIconUri(def.ImagePath))
            );
        ...
        private static string ToPackageIconUri(string? value)
        {
            string iconPath = string.IsNullOrWhiteSpace(value)
                ? Constants.Constants.DefaultImageRelativePath
                : value.Trim();

            if (iconPath.StartsWith("ms-appx:", StringComparison.OrdinalIgnoreCase))
            {
                iconPath = iconPath["ms-appx:".Length..];
            }

            iconPath = iconPath.TrimStart('\\', '/').Replace('/', '\\');
            return $"ms-appx:{iconPath}";
        }

        private static string FromPackageIconUri(string value)
        {
            string iconPath = value.Trim();
            if (iconPath.StartsWith("ms-appx:", StringComparison.OrdinalIgnoreCase))
            {
                iconPath = iconPath["ms-appx:".Length..];
            }
