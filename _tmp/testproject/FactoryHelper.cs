    ...
    internal sealed class FeedProviderClassFactory
    {
        ...
        public int CreateInstance(nint pUnkOuter, in Guid riid, out nint ppvObject)
        {
            ...
            try
            {
                ...
                string interfaceProbe = string.Join(
                    "; ",
                    ProbeInterface(ppvObject, ComGuids.IFeedProvider, "IFeedProvider"),
                    ProbeInterface(ppvObject, new Guid("AF86E2E0-B12D-4C6A-9C5A-D7AA65101E90"), "IInspectable"),
                    ProbeInterface(ppvObject, new Guid("94EA2B94-E9CC-49E0-C0FF-EE64CA8F5B90"), "IAgileObject"),
                    ProbeInterface(ppvObject, new Guid("00000003-0000-0000-C000-000000000046"), "IMarshal"));
                File.AppendAllText(
                    Path.Combine(Path.GetTempPath(), "FeedProvider-marshal-latest.log"),
                    $"iid={riid}; result={ppvObject}; {interfaceProbe}{Environment.NewLine}");
                return HResults.SOk;
            }
            catch (Exception ex)
            {
        ...
        private static string ProbeInterface(nint source, Guid iid, string name)
        {
            int result = Marshal.QueryInterface(source, ref iid, out nint pointer);
            if (pointer != nint.Zero)
            {
                Marshal.Release(pointer);
            }

            return $"{name}=0x{result:X8}";
        }

        public int LockServer(int fLock) => HResults.SOk;
    }

    internal sealed unsafe class ProviderComWrappers : ComWrappers
    {
        internal static ProviderComWrappers Instance { get; } = new();
            ...
            out int count)
        {
            ...
            ComInterfaceEntry* entries = (ComInterfaceEntry*)RuntimeHelpers.AllocateTypeAssociatedMemory(
                typeof(ProviderComWrappers),
                sizeof(ComInterfaceEntry));
            entries[0] = new ComInterfaceEntry
            {
                IID = ComGuids.IFeedProvider,
                Vtable = ABI.Microsoft.Windows.Widgets.Feeds.Providers.IFeedProviderMethods.AbiToProjectionVftablePtr
            };
            return entries;
        }
    ...
    internal sealed unsafe class FactoryComWrappers : ComWrappers
    {
            ...
            nint* result)
        {
            ...
            try
            {
                ...
                return Factory.CreateInstance(outer, in requestedIid, out *result);
            }
            catch (Exception ex)
            {
                *result = nint.Zero;
