...
namespace FeedCustomizer.Core.Tools
{
    public class ResourcesCopier
    {
        ...
        public static async Task ResourcesCopyAsync()
        {
            ...
            try
            {
                // Refreshing the packaged template must not erase user-defined
                // feed definitions from the writable registration manifest.
                ...
                RemoveLegacyProviderDirectories(UserAppFolder);
        ...
        private static void SynchronizeManifestExecutablePath()
        {
                ...
                .Where(element => element.Attribute("Id")?.Value == "feedcustomizer"))
            {
                ...
                if (provider is not null)
                {
                    ...
                    provider.SetAttributeValue("Icon", "ms-appx:Assets\\StoreLogo.scale-200.png");
                    provider.Attribute("SettingsUri")?.Remove();
                }
            }
        ...
        private static async Task SynchronizeProviderFilesAsync()
        {
            ...
            try
            {
                ...
                if (File.Exists(AppDataPaths.ManifestPath))
                {
                    ...
                    {
                        Debug.WriteLine($"Failed to migrate existing feed definitions: {ex.Message}");
                    }
                }
                ...
                if (existingFeeds is { Count: > 0 })
                {
                    // Re-serialize the existing feeds after a package update so
                    // old manifests receive the required provider identity and
                    // the current ms-appx icon URI format.
                    await ManifestXmlService.Write(existingFeeds, AppDataPaths.ManifestPath);
                }
            }
            finally
        ...
        private static void RemoveLegacyProviderDirectories(string rootFolder)
        {
            string providerRoot = Path.Combine(rootFolder, "FeedProvider");
            foreach (string legacyFolder in new[] { "win-x64", "win-arm64" })
            {
                string path = Path.Combine(providerRoot, legacyFolder);
                ...
                {
                    Debug.WriteLine($"Failed to remove legacy provider folder {path}: {ex.Message}");
                }
            }
        }
        ...
        public static bool IsRegisteredProviderCurrent()
        {
            ...
            string[] requiredFiles = ["FeedProvider.exe"];
